...
const Chat = () => {
	const { users, userAuthHeader, userSelected, channelSelected, channelDetails, chatType, chat, setChat, setAddMemberType } = useContext(UserContext);
															...
															return (
																<div key={index}>
																	<div className="flex items-center bg-base-300 justify-center w-7 h-7 -mx-1 overflow-hidden rounded-xl border-2 border-white">
																		<span>{!getUid(users, member.user_id)? "" : getUid(users, member.user_id).toUpperCase().charAt(0)}</span>
																	</div>
																</div>
															)
														}
